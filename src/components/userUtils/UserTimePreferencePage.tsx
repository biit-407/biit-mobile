...
import { FlatList, GestureResponderEvent, Modal, StyleSheet } from "react-native";
...
import { UserTimePreferencePageNavigationProp, UserTimePreferencePageRouteProp } from "../../routes";
...
export default function UserTimePreferencePage({
    ...
    const [preferences, setPreferences] = React.useState<{ start: number, end: number }[]>([]);
    const [show, setShow] = React.useState(false)
    const [dateState, setDateState] = React.useState({ state: 0, start: 0, end: 0, index: 0, day: 0 })
    ...
    const updateTimePreference = (index: number) => {
        ...
        setDateState({ ...dateState, start: startTime, end: endTime, state: 0, index: index })
        setShow(true)
    }
    ...
    const renderUserPreference = ({ item, index }: { item: { start: number, end: number }; index: number }) => {
        return <UserTimePreferenceCard
            index={index}
                ...
                <Box style={{ alignSelf: 'center', width: '80%' }}>
                    ...
                    <Picker
                        ...
                        onValueChange={(itemValue: React.ReactText, _itemIndex: number) => {
                            setDateState({ ...dateState, day: parseInt(itemValue.toString()) })
                        }}
                    ...
                    <Button title={'Submit'} onPress={() => {
                        ...
                        copy[dateState.index] = { start: dateState.start + offset, end: dateState.end + offset }
                        setPreferences([...copy])
                    }} />
                </Box>
