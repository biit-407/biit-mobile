...
export default function HomePage({ route }: HomePageProps) {
  ...
  const load = (futureIDs: string[], tentativeIDs: string[]) => {
    ...
    if (futureIDs.length > 0) {
      for (const meetupID in futureIDs) {
        ...
        ml.push({
          ...BLANK_MEETUP,
          id: futureIDs[meetupID],
          meetupType: "accepted" as MeetupType,
        });
      }
    } else {
    ...
    if (tentativeIDs.length > 0) {
      for (const meetupID in tentativeIDs) {
        ...
        ml.push({
          ...BLANK_MEETUP,
          id: tentativeIDs[meetupID],
          meetupType: "tentative" as MeetupType,
        });
      }
    } else {
  ...
  const renderMeetup = ({
    ...
    return (
      <>
        {isTextItem(item) ? (
          <Text variant={(item as TextItem).variant} padding="md">
