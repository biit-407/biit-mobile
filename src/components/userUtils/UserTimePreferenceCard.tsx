...
const UserTimePreferenceCard = ({ index, startTime, endTime, deleteCallback, updateCallback }: UserTimePreferenceCardProps) => {
    const dayOfWeek = getDayOfWeek(startTime)
    const startTimeString = `${startTime % 12}:00 ${startTime%24 >= 12 ? 'PM' : "AM"}`
    const endTimeString = `${endTime % 12}:00 ${endTime%24 >= 12 ? 'PM' : "AM"}`
    ...
    return (
        <ThemedCard>
            <Box style={{ flexDirection: "row", justifyContent: "space-between" }}>
                ...
                <Box>
                    <Text variant="header" style={{alignSelf: 'flex-end'}}>{startTimeString} - {endTimeString}</Text>
                    <Box style={{ paddingTop: theme.spacing.md, flexDirection: "row", justifyContent: "flex-end" }}>
                        <Button
