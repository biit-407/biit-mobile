...
type ActionType = 'set grant token' | 'set tokens' | 'set user info' | 'invalidate'
type Action = {
    type: ActionType;
    grantToken: string;
    accessToken: string;
...
const AzureStateContext = React.createContext<AzureState | undefined>(undefined)
const AzureDispatchContext = React.createContext<Dispatch | undefined>(undefined)
...
function azureReducer(state: AzureState, action: Action): AzureState {
    switch (action.type) {
        ...
        case 'invalidate': {
            return { ...state, userInfo: BLANK_AZURE_USER_INFO, grantToken: '', refreshToken: '', accessToken: '' }
        }
    }
}

function AzureProvider({ children }: AzureProviderProps) {
    const [state, dispatch] = React.useReducer(azureReducer, { grantToken: '', accessToken: '', refreshToken: '', userInfo: BLANK_AZURE_USER_INFO })
    return (
...
class AzureClient {
    ...
    public static async requestTokens(grantToken: string): Promise<{ refreshToken: string, accessToken: string }> {
    ...
    public static async requestUserInfo(accessToken: string): Promise<AzureUserInfo> {
...
async function requestTokens(dispatch: Dispatch, state: AzureState) {
    try {
        ...
        dispatch({...state, type: "set tokens", refreshToken: tokens.refreshToken, accessToken: tokens.accessToken })
    } catch (error) {
